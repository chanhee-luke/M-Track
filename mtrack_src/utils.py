...
def load_datasets(args):
    '''
    load ALFRED dataset and
    create queue of trajectories to be extracted
    '''
...
def pad_instr_tokens(high_instr_tokens, low_instr_tokens, maxlength=20):

    if len(high_instr_tokens) + len(low_instr_tokens) <= 2: #assert len(raw_instr_tokens) > 2
        return None

    #TODO Check how much instrs combo are greater than maxlength
    if len(high_instr_tokens) + len(low_instr_tokens) > maxlength - 2: # -2 for [CLS] and [SEP]
        print(f"Max length low! actual length {len(high_instr_tokens) + len(low_instr_tokens)}")
        print(high_instr_tokens)
        print(low_instr_tokens)
        print()
        #raise Exception(f"maxlength too low! Only {maxlength} while instrution size is {len(high_instr_tokens) + len(low_instr_tokens)}")
        high_instr_tokens = high_instr_tokens[:(maxlength-len(low_instr_tokens)-2)]

    instr_tokens = ['[CLS]'] + high_instr_tokens + ['[SEP]'] + low_instr_tokens
    num_words = len(high_instr_tokens) + len(low_instr_tokens) + 2 # include [SEP]
    instr_tokens += ['[PAD]'] * (maxlength-num_words)
    ...
    if not test_only:
        ...
        with open(feature_store, "r") as tsv_in_file:     # Open the tsv file.
            ...
            for item in reader:
                ...
                # print(np.shape(np.frombuffer(base64.decodestring(item['features'].encode('ascii')),
                #                                    dtype=np.float32)))
                # print(np.shape(features[long_id]))
                # exit()
    else:
        features = None
