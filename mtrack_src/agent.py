...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20):
        ...
        # Models
        #TODO Load detectron here too, but keep it frozen
        #TODO Load target tagger too, but keep it frozen
        ...
        self.vis_model = VisionEncoder(args) # Frozen
    ...
    def _sort_batch(self, obs, hidx):
        assert len(obs) == len(hidx) # Sanity check
        seq_tensor = np.array([ob['instr_encoding'][idx] for ob, idx in zip(obs, hidx)])
    ...
    def get_input_feat(self, obs):
        ...
        for i, ob in enumerate(obs):
            input_a_t[i] = utils.angle_feature(ob['rotation'], ob['horizon'])
    ...
    # Change this to do multiple action on the ALFRED simualtor given the view
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        sentence, language_attention_mask, token_type_ids, \
            seq_lengths, perm_idx = self._sort_batch(obs, self.env.sims["hidx"])
        perm_obs = obs[perm_idx]
        hidx = np.array(self.env.sims["hidx"])
        perm_hidx = hidx[perm_idx]
        ...
        traj = [{
            'instr_id': ob['instr_id'][hidx],
            'path': []
        } for ob, hidx in zip(perm_obs, perm_hidx)]
        ...
        for t in range(self.episode_len):
            ...
            # Get image features from Detectron
            ...
            if train_rl:
                ...
                #TODO Change rewards, on demand distance
