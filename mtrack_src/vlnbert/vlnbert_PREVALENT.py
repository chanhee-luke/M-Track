...
class VisionEncoder_og(nn.Module):
...
class VisionEncoder(nn.Module):
    def __init__(self, vision_size, obj_size, config):
        super().__init__()
        feat_dim = vision_size
        obj_feat_dim = vision_size

        # Image feature encoding
        self.visn_fc = nn.Linear(feat_dim, config.hidden_size)

        # Object feature encoding
        self.obj_fc = nn.Linear(obj_feat_dim, config.hidden_size)

        self.visn_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, visn_input, obj_input):
        feats = visn_input
        obj_feats = obj_input
        ...
        o = self.obj_fc(obj_feats)
        o = self.visn_layer_norm(o)

        obj_output = self.dropout(o)
...
class VLNBert(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None, attention_mask=None,
        lang_mask=None, vis_mask=None, position_ids=None, head_mask=None, img_feats=None, obj_feats=None):
        ...
        if mode == 'language':
            ...
            img_feats = self.vision_encoder(img_feats, obj_feats)
